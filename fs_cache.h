...
//   [{"type":"dir|file","name":"..","size":N},...]（已应用 ofsBlacklistedEntry/ofsWhitelistedFile 过滤）。
...
#define FS_CACHE_MAX_ITEMS 50     // 每目录条目上限（与 /fs/list count 上限一致）
...
// /fs/list 用: 读指定目录的 LittleFS 缓存, 按 start/count 切片（照抄官方 handleFileList 的
// chunkedResponseModeStart + 复用 String + sendContent 分块）; 缓存不存在 → 返回 false（调用方 500）。
// 注意: 入参 dir 已规范化; CPU/堆峰值只与 count 相关（全文件读入再切, 但每目录 ≤50 项不超预算）。
