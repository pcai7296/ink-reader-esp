...
| key | 类型 | 语义 |
|---|---|---|
| `file` | string ≤64B | 文件名（UTF-8）。`PUT /progress` body 中**必带**（设备总是附加，App 用它校验目标书）；`GET /progress` 响应中**可省**（请求本身已携带 `file` 参数，响应回显无信息量，且会挤占 §3.4 的 256B 指纹预算——2026-09-13 由"必须携带"修订） |
...
`offset > size_local`（手机侧文件较大）**同样必走 pct 换算**（直读会越界；2026-09-13 前该场景被误报
"手机进度无效"）；其余情况直接用精确字节偏移（误差 <1 页）。
...
- v4.1（2026-09-13）：§3.3 GET 响应 `file=` 由"必须携带"降为**可选**（请求已含 file 参数，回显无信息量且挤占
  256B 指纹预算）；§4.1 明确 `offset > size_local` **必走 pct 换算**（原固件 PARSE 硬门误报"手机进度无效"）。
  `/sync_bind.dat` 位置勘正为 **LittleFS 根**（§2.1，原误写"SD 根"）；`dev_ip=` 空串回落 DHCP 为正式语义。
