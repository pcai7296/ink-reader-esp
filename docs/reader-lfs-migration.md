...
> ⚠️ **已废弃（本节仅作历史留痕）**：同日 §16 按用户拍板"抄官方"改为**媒体跟随**（阅读数据源 = 当前介质），
> "点 SD 上的 TXT = 先导入内部"的形态被撤销；`readerImportFromSd()` 函数已在 2026-09-12 全库审查轮**删除**
> （死代码）。下文描述的 `IMPORT_*` 日志/前置校验均不再存在于固件中。
...
- ~~保留 `readerImportFromSd()`（>100MB/容量前置校验 + 回读校验 + 旁带复制）作为**可选**"拷到内部"能力~~
  —— **2026-09-12 全库审查轮已删除该函数（死代码，无调用点）**；当前没有"拷到内部"入口，SD 书就是
  在 SD 上读、索引也建在 SD。若将来需要"拷到内部"，按本节校验规则重新实现。
