...
- **📺 天气页正中显示什么（2026-09-12 用户拍板：由我选一个天气相关信息）**：最终选
  **室内温湿度(板载 SHT30)** ——
  与天气/气候相关、不重复本页已有字段（大号 7 段数字=室外温度、右上=室外湿度），且正好用上传感器。
  布局 `[房子][温度计]29℃ [水滴]59%`（房子=室内标识），在 `[64,292]` 内居中；**无传感器自动回落 InAWord**
  （一言/自定义句/倒计时/B粉，官方 V14 同款）。实机数字自检 `-DWEATHER_RENDER_TEST=1`（造假天气数据走完整
  渲染并打印布局）：`strip indoor avail=228 total=117 sx=119 end=236 fit=1`（电量占 [4,56]，不重叠不截断），
  两遍渲染(全刷+局刷)无异常。**候选备选（换一个只是改这段内容）**：生活指数扩展（穿衣/洗车/感冒，需扩
  `life/suggestion.json` 解析 + 新图标）、降水概率/日出日落（需扩 `daily.json` 解析）、室内外温差对比、
  保持官方 InAWord；空气质量 AQI 不在心知天气接口内需另找数据源。
- **🔧 天气页中间条与水滴图标修正（2026-09-12 用户反馈）**：
  ① **天气页正中不该是电量**（用户："为什么天气页面最中心是与天气无关的电量"）——查官方 V14
  `DisplayMain.ino:257-301`：双横线 y=56/74 之间放的是 **InAWord**（一言/自定义句/倒计时/B粉），
  而**电量在角落**（左下角 21×12 电池位图 + `u8g2_font_u8glib_4_tf` 4px 字体，`setCursor(4,113)`）。
  我们的三列预报占满底部（y=77..131），故：**正中改为 `clockBuildSubText()` 的 InAWord 内容 + 事件图标**
  （旗子/B站，复用时钟页同款）；**电量紧凑放中间条左端**（图标 x=4 + `%` 文字 x=24，`subLeft=64`），
  其余宽度给 InAWord 居中 —— 与电量永不重叠（InAWord 过长时按 `maxW` 截断）。无文案且夜间跳过 → "夜间不更新"。
  **⚠️ 正中内容随后按用户要求再改为"室内温湿度"，InAWord 降级为无传感器时的回落项 —— 见上面 📺 条（以那条为准）。**
