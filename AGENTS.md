...
- **⛔ 阅读架构定稿（2026-09，用户拍板，勿回退）**：阅读实时链路**只用 LittleFS**（正文/.i1/.z1/阅读状态/标签/休眠快照），**SD 与 WiFi 退出阅读**。实现=`readerFs()`(=LittleFS) + `readerBusReady()` 占位（阅读链路不再做 SD 总线仲裁）；`startTxtReader` 仅在 `gBrowseLocal`（内部介质）下工作，SD 介质点开 TXT → 提示“SD 仅文件管理”。写入模式严格按 `fs::FS`：读 `"r"`/新建重写 `"w"`/**追加必须 `"a"`**（SD 的 `FILE_WRITE` 是追加，照搬会截断 `.i1/.z1/.bm`）。仍在 SD 的仅：`debug_trace.log`(DIAG_SD)、天气缓存、文件管理与介质切换的 `SD.begin`。容量边界：LittleFS≈1.98MiB，索引≈正文×2.07% ⇒ 单本正文上限≈1.8–1.9MB；>100MB 一律拒绝。计划/验收 A–E 见 `docs/reader-lfs-migration.md`；测试书用**现有 Web 文件管理直接上传进 LittleFS**（切内部介质, 不做导入/SD 中转）。
