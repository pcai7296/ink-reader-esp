...
class ReadBookActivity : BaseReadBookActivity(),
    ...
    override fun onResume() {
        ...
        // ESP 推送确认闭环：设备推送到达时若 App 在后台只发了通知，回到阅读界面补弹确认框
        runCatching { io.legado.app.esp.EspSyncDialog.showPendingIfAny() }
        if (bookChanged) {
            bookChanged = false
...
//                    }
                    ...
                    // ESP 同步：√ 状态 = 同步服务开关
                    R.id.menu_esp_sync -> item.isChecked = io.legado.app.esp.EspSyncConfig.enabled
                }
            }
        }
    ...
    private fun toReaderPopupItem(item: MenuItem): PopupAction.PopupActionItem {
        val isEspSync = item.itemId == R.id.menu_esp_sync
        val espEnabled = isEspSync && io.legado.app.esp.EspSyncConfig.enabled
        return PopupAction.PopupActionItem(
            // ★ 2026-09-13 用户要求"开关状态由 √ 显示"：实测 PopupAction 的勾选列在小屏上不可靠，
            //   改为**标题内嵌 √**（100% 可见，也便于自动化断言）。
            title = if (espEnabled) "√ ${item.title}" else item.title?.toString().orEmpty(),
            value = ACTION_READER_ITEM_PREFIX + item.itemId,
            icon = item.icon?.constantState?.newDrawable()?.mutate(),
            enabled = item.isEnabled,
            checkable = item.isCheckable,
            // ESP 同步的 √ 以服务配置为唯一真源（菜单构建时序可能让 item.isChecked 滞后一拍）
            checked = if (isEspSync) io.legado.app.esp.EspSyncConfig.enabled else item.isChecked
        )
    }
...
//            }
            ...
            // ESP 同步：直接启停同步服务（√ = 开）；走 EspSyncConfig.requestEnabled 唯一入口
            // （守卫：未连 Wi-Fi / 开热点 → 拒绝；并广播事件让设备页 Switch 立即同步）
            R.id.menu_esp_sync -> {
                val target = !io.legado.app.esp.EspSyncConfig.enabled
                if (io.legado.app.esp.EspSyncConfig.requestEnabled(this, target)) {
                    toastOnUi(if (target) "ESP 同步已开启" else "ESP 同步已关闭")
                } else {
                    toastOnUi(io.legado.app.esp.EspSyncConfig.denyReason() ?: "无法开启同步")
                }
                menu?.findItem(R.id.menu_esp_sync)?.isChecked =
                    io.legado.app.esp.EspSyncConfig.enabled
            }
