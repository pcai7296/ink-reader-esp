...
enum class ReaderMenuItem(
    ...
    EspSync("espSync", intArrayOf(R.id.menu_esp_sync), R.string.esp_sync_enable_title),
...
fun loadReaderMenuConfig(context: Context): ReaderMenuConfig {
    ...
    val config = ReaderMenuConfig.fromJson(json).normalized()
    // 用户要求：点"更多选项"后弹出的列表（primary）第一位固定为 ESP 同步。
    // （二级 more 列表在手表上因 primary 占满屏幕而永远点不到，不放这里）
    val primary = listOf("espSync") + config.primary.filter { it != "espSync" }
    val more = config.more.filter { it != "espSync" }
    return if (config.primary == primary && config.more == more) config
    else ReaderMenuConfig(primary, more)
}
