...
object PreferKey {
    ...
    const val showSourceCheckStatus = "showSourceCheckStatus"
    const val blockSourceNavigation = "blockSourceNavigation"
    ...
    const val coverAuthorLargeSize = "coverAuthorLargeSize"
    const val coverAuthorSmallSize = "coverAuthorSmallSize"
    ...
    const val readAloudControlsPause = "readAloudControlsPause"
    const val readAloudControlsPosition = "readAloudControlsPosition"
    const val readAloudControlsAutoHide = "readAloudControlsAutoHide"
    ...
    const val readAloudControlsY = "readAloudControlsY"
    ...
    const val importRememberGroup = "importRememberGroup"
    const val importLastGroup = "importLastGroup"
    const val importLastGroupAdd = "importLastGroupAdd"
    ...
    const val mouseWheelScrollSpeed = "mouseWheelScrollSpeed"
    ...
    const val showBookMemo = "showBookMemo"
    ...
    // ESP 墨水屏进度同步（自 io.legado.app.esp 移植）
    const val espSyncEnable = "espSyncEnable"

    // 手表模式（小屏 + 物理圆角适配）
    /** 当前模式（首次启动由自动识别写入，之后反映用户选择） */
    const val watchMode = "watchMode"
    /** 首次自动识别是否已完成（只表示"检测跑过了"，不参与后续重新识别） */
    const val watchModeFixed = "watchModeFixed"
}
