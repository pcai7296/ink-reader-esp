...
object AppConfig : SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    /**
     * 手表模式（小屏 UI 适配）。
     * 首次启动由 WatchMode.ensureFixedOnFirstLaunch() 写入识别结果；之后只反映用户选择。
     */
    var watchMode = appCtx.getPrefBoolean(PreferKey.watchMode, false)
    ...
    override fun onSharedPreferenceChanged(sharedPreferences: SharedPreferences?, key: String?) {
        when (key) {
            PreferKey.watchMode -> watchMode = appCtx.getPrefBoolean(PreferKey.watchMode, false)
